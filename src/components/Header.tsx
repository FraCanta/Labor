...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-primary/10 bg-white shadow-[0_1px_12px_rgba(7,86,107,0.06)]">
      <div className="mx-auto flex h-20 w-full max-w-[1400px] items-center justify-between px-5 sm:px-8 lg:h-[88px] lg:px-10">
        <a
          href="#hero"
          onClick={closeMenu}
          className="inline-flex shrink-0"
          aria-label="LABOR Patronato e CAF, torna all'inizio"
        >
          <Image
            src="/labor-logo2.svg"
        ...
        >
          {navigation.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-base font-bold text-primary-dark transition-colors hover:text-primary focus-visible:text-primary"
            >
              {item.label}
            </a>
          ))}
      ...
      >
        {navigation.map((item) => (
          <a
            ...
            href={item.href}
            onClick={closeMenu}
            className="border-b border-primary/10 py-4 font-heading text-base font-bold text-primary-dark"
          >
            {item.label}
          </a>
        ))}
        <a
          href="tel:0802376085"
          onClick={closeMenu}
          className="mt-5 inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-accent px-5 font-heading text-sm font-bold uppercase tracking-[0.04em] text-primary-dark"
        >
          <Icon icon="lucide:phone" aria-hidden="true" className="text-lg" />
          Chiama ora
        </a>
